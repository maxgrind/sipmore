...
void UpdServer(void)
{
	...
	ulMode = 1;
	ioctisocket(s, FIONBIO, (unsigned long*)&ulMode);
...
int UdpServerProcess(SOCKET socket, char* pBuf, struct sockaddr_in * si_input)
{
	//struct sockaddr_in  si_other;
	//int slen = sizeof(si_other);
	int slen = sizeof(*si_input);
	WSADATA wsa;
	int recv_len;

	//try to receive some data, this is a blocking call
	recv_len = recvfrom(socket, pBuf, BUFLEN, 0, (struct sockaddr *) &si_input, &slen);
...
#ifdef M_HOME_PC 
	inaddr.S_un.S_addr = 0x2101a8c0; // to "192.168.1.33"
#else
	//inaddr.S_un.S_addr = 0x2298a8c0; // to "192.168.152.34"
	//inaddr.S_un.S_addr = 0x2101a8c0; // to "192.168.1.33"
	inaddr.S_un.S_addr = 0x012ba8c0; // to "192.168.43.1"
	//inaddr.S_un.S_addr = 0x0501a8c0; // to "192.168.1.5"
#endif
